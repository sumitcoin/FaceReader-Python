...
def get_windows_camera_names():
    """Return DirectShow camera names when the optional pygrabber is installed."""
    try:
...
def discover_cameras():
    """Find connected cameras and return (index, display name) pairs."""
    names = get_windows_camera_names()
    cameras = []
    backend = cv2.CAP_DSHOW if hasattr(cv2, "CAP_DSHOW") else cv2.CAP_ANY

    # Named DirectShow devices map to OpenCV indices in the same order.
    indices_to_check = range(len(names)) if names else range(MAX_CAMERA_INDEX)
    ...
    for index in indices_to_check:
        ...
        if available:
            # Reading a frame prevents unavailable virtual devices being listed.
            available, _ = test_camera.read()
        test_camera.release()
...
def select_camera():
    ...
    if not cameras:
        raise RuntimeError(
            "No working camera was found. Check camera permissions and connections."
        )

    print("\nAvailable cameras:")
    for menu_number, (camera_index, name) in enumerate(cameras, start=1):
        print(f"  {menu_number}. {name} (camera index {camera_index})")

    if len(cameras) == 1:
        print(f"\nAutomatically selected: {cameras[0][1]}")
        return cameras[0]
    ...
    while True:
        ...
        try:
            ...
            pass
        print("Invalid selection. Enter one of the numbers shown above.")
...
def main():
    ...
    camera_index, camera_name = select_camera()
    backend = cv2.CAP_DSHOW if hasattr(cv2, "CAP_DSHOW") else cv2.CAP_ANY
    ...
    try:
        while True:
            ...
            cv2.imshow(f"Automatic Face Capture - {camera_name}", display)
